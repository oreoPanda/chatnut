...
extern void show_message_history( const char *history )
{
	...
	if( history )
	{
		gtk_text_buffer_set_text( history_buffer, history, -1 );	//-1 cuz text is terminated
	}
	else
	{
		gtk_text_buffer_set_text( history_buffer, "<No recent chats found>", -1 );	//-1 cuz text is terminated
	}
...
extern void enable_input_view( gboolean (*key_pressed_cb)( GtkWidget *, GdkEvent *, gpointer ) )
{
	...
	g_signal_connect( input_view, "key-press-event", G_CALLBACK(key_pressed_cb), history_view );
