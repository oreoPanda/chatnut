...
/*int main( int argc, char *argv[] )
{
	...
	char *cmd_buffer = malloc(CMD_LEN);
	fd_set readfds;
	fd_set writefds;
	int maxfds;		//number of highest file descriptor
	struct timeval timeout;
	int conn_accepted = FALSE;
	...
	int sign;
	while( ( sign = getch() ) != 'q' )		//start messaging loop
	{
		//if connection was not yet accept()ed, check for conn_msg from server to show that client is really connected to the messaging part of the server, not just the adress
		if( conn_accepted == FALSE )
		{
			reset_timeout( &timeout );
			FD_SET( sock, &readfds );
			my_select( maxfds+1, &readfds, NULL, NULL, &timeout );
			if( FD_ISSET( sock, &readfds ) )
			{
				FD_CLR( sock, &readfds );
				TCP_recv( &sock, msg_buffer, MSG_LEN, 0, NULL );		//FIXME NULL and the previous select() check are confusing :D
				if( strncmp( msg_buffer, conn_msg, CONN_MSG_LEN-1 ) == 0 )
				{
				...
				{
					error_exit( "Broken conn_msg\n" );
				}
			}
			else
			{
				continue;	   //skip messaging and check for conn_msg again
			}
			printf( "Welcome! Press / to enter a command or m to enter a message. Use /help for more info.\n" );
		}
		
		//check for inbound messages and check if the server is still there
		reset_timeout( &timeout );
		FD_SET( sock, &readfds );
		my_select( maxfds+1, &readfds, NULL, NULL, &timeout );
		if( FD_ISSET( sock, &readfds ) )
		{
			FD_CLR( sock, &readfds );
			int retval = TCP_recv( &sock, msg_buffer, MSG_LEN, 0, NULL );   //FIXME nother bad NULL
			if( retval == 0 )
			{
		...
		//if wanted, send messages
		if( sign == 'm' )
		{
			if( name_set && receiver_set )//if name and receiver is set, continue with sending
			{
				get message
				printf("Please enter a message: ");//FIXME watch out that this happens every time (fflush() because no \n?)
				fgets( msg_buffer, MSG_LEN, stdin );
				cmd_buffer[strlen(msg_buffer)-1] = '\0';//replace \n with \0
				printf("Will now send...\n");

				send message FIXME might block for some time, possible hang point
				TCP_send( &sock, msg_buffer, strlen(msg_buffer)+1, 0 );
			}//end send if receiver and name is set
			else//no receivers set yet
			{
				printf("You have to set your name with /name and at least one buddy with /who first!\n");
			}
		}//end sending messages

		if( sign == '/' )
		{
			//get command TODO add a status message that gets displayed when the command+arguments string was too long
			printf("Please enter a command: /");//FIXME watch out that this happens every time (fflush() because no \n?)
			*cmd_buffer = '/';
			fgets( cmd_buffer+1, CMD_LEN-1, stdin );
			cmd_buffer[strlen(cmd_buffer)-1] = '\0';//replace \n with \0
			TCP_send( &sock, cmd_buffer, strlen(cmd_buffer)+1, 0 );
			//command+arguments structure is separated on the server side, as long as it's not longer than CMD_LEN the transmission works

			//see if the command worked
			TCP_recv( &sock, msg_buffer, MSG_LEN, 0, NULL );//wait for reply (reply will come eventually, don't check if read will block)
			eval_reply( msg_buffer );
		}//end '/' evaluation
	}//end messaging loop
